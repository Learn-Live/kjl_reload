...
for dataset in UNB345_3 CTU1 MAWI1_2020 MACCDC1 SFRIG1_2020 AECHO1_2020 DWSHR_WSHR_2020; do
	for model in "OCSVM(rbf)" "KJL-OCSVM(linear)" "Nystrom-OCSVM(linear)" "KJL-GMM(full)" "KJL-GMM(diag)" "Nystrom-GMM(full)" \
		...
		#  cd examples/
		#  echo ${dataset}
		echo 'PYTHONPATH=../:./ python3.6 -u speedup/main_kjl.py -d' ${dataset}' -m '${model}' > speedup/out/main_kjl_'${dataset}'_'${model}'.txt 2>&1 &'
		PYTHONPATH=../:./ python3.6 -u speedup/main_kjl.py -d ${dataset} -m ${model}> speedup/out/main_kjl_${dataset}_${model}.txt 2>&1 &
		# echo ${dataset}
